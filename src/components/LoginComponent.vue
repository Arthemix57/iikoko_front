<template>
    <div class="fixed inset-0 flex items-center justify-center bg-black/10 backdrop-blur-md flex-col z-50">
        <alert-component v-if="alert.message.length > 0" :type="alert.type" :message="alert.message" icon :duration="5000" />
    ...
    methods: {
        login() {
            ...
            if (this.pin.length < 0) {
                ...
                this.loading = true;
                this.axios.post(`${this.api}loginCaisse`, { code_pin: this.pin })
                    .then(({ data }) => {
